	...
	<div class="wrapper">
		...
		<div class="container">
			...
			<div class="pracProgress_hdr">
				<h2>Geo Location Redirection</h2>
			...
			<div class="ur_grid ur_grid_pop p-font14">
				...
				<table class="table table-striped table-hover table-bordered"
					...
					<tbody id="mapStats">
						<c:forEach items="${statesList}" var="state" varStatus="count">
							<tr>
								<td>${state}</td>
								<td id="list${count.index}"><c:forEach items="${savedlList}" var="list2"
										varStatus="counter">

										<c:if test="${list2[0] eq state}">
									       ${list2[1]}
									      </c:if>
								...
								<td><select id="selectValue${count.index}"
									name="selectValue${count.index}" onchange="saveType(this,'list${count.index}')">
										<option value="0">Move to A/B</option>
										<option value="${state}">A</option>
										<option value="${state}">B</option>
								</select></td>
...
function saveType(sel, index){
	...
	var typeState =  sel[sel.selectedIndex].value;
	//alert(typeState);
	//if(parseInt(typeState)!==0){
	$.ajax({
		url : "<%=request.getContextPath()%>/saveTestType",
		data : {type:type,
			typeState:typeState
		},
		method : 'POST',
