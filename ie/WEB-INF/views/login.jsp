...
<body>
	...
	(function($){
		...
		setTimeout(function(){ $("#loginError").remove(); }, 10000);
...
if($("#f").valid()){
                	 ...
                	 $.each( response.modelMap.categories, function(index,value) {
                		 ...
                		 if(sizeOfArrray>1){
                			 if(value.categoryId==3)
                    			 ...
                    			 '<a onclick="submitFormWithRoleAndSchool('+value.categoryId+','+value.schoolId+');" class="hero_role'+value.categoryId+'"></a><p>'+value.schoolName+'</p></li>';
                				 }
                			 else
                				 {
                			 userOptions+='<li class="width33 role-select mbtm30">'+
		...
		function submitFormWithRoleAndSchool(roleId,schoolId) {
			$("#j_password").val($("#password").val());
			$("#email").val($("#username").val() + ";" + roleId+";"+schoolId);
			$("#f").submit();
		}
	</script>
	<script>
