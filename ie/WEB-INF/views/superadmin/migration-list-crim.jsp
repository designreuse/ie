           ...
           <h2>Manage Users</h2>
            ...
            <!--TAB 1 STARTS-->
                <div id="tab1">
                	...
                	<input type="button" value="All Users" id="tab-2" class="table-head-btns-slctd" onclick="getAllTeachers(2);"/>
            			<!-- <input type="button" value="Schools" id="tab-3" onclick="getAllTeachers(3);"/> -->
            		</div>
...
<%--  Migrated Joined User count = ${dtoUser.migratedJoinedCount}, Migrate Not Joined User count = ${dtoUser.migratedNotJoinedCount} --%>
 ...
 <div class="function-list padding-top-zero" id="joinedNotJoined" style="display:none">
                	<ul>
                 ...
                 <div class="function-list padding-top-zero" id="activeNotActive">
                	<ul>
...
<table class="table table-striped table-bordered table-hover" id="sample_1">
    						...
    						<tr class="odd gradeX">
								...
								<td id=status-${migrated.userId}>
									${migrated.roleType}
								</td>
								<td>
									 
									 <a onclick="setTeacherProgress('${migrated.email}',${migrated.userId});">[Set Progress] </a>
									 <a onclick="deleteTeacher(${migrated.userId});" id=deleteStatus-${migrated.userId}>[Delete]</a>
									...
									<a onclick="setChangePassword('${migrated.email}',${migrated.userId});"> [Edit]</a>
								</td>
							</tr>
    						</c:forEach>
        ...
        <div id="adminLoader" style="display: none; margin-top: 300px;" align="center"> <img src="<%=request.getContextPath()%>/NewStyles/images/ajax-loader-crim.gif"  alt=""/> </div>
     ...
     <div class="containerSmall" id="teachersignup" style="display:none">
				<form:form role="form" 
					...
					modelAttribute="dtoUser" method="post">
						...
						<input type="hidden" value =0 name="schoolId" id="tSchoolId" />
						<ul>
						<li>
								<div class="contact-row">
									<div class="outer-field-wrap">
...
<h3 class="migChngPass">Edit</h3>
<label>Email: </label><input type="text" id="editEmail" name="Email"/>
...
<div class="ourStory-Popup"  id="userPerDay" style="display: none;">
    ...
    <div id="deleteTeacher" style="display:none; width:160px;">
     	<p>Teacher has been deleted</p>
     	<center><button onClick="closePopup();" class="table-head-btns-slctd">Ok</button></center>
     	</div>
     	<div id="editDetails" style="display:none; width:160px;">
...
function principalSignUp(){
	...
	$("#parentSignUp").hide();
				...
				function getAllTeachers(userRoleId){
					...
					$("#sample_1").dataTable().fnDestroy();
					$("#sample_1").html('');
					var url='<%=request.getContextPath()%>/superadmin/getAllTeachers-crim';
					$.ajax({
						url:url,
						method:'GET',
						async :true,
								 ...
								 '<th>Action</th>'+
									...
									$.each(response.modelMap.migrated, function(index,value) {
										if(userRoleId==1){
											...
											teacherDetail+='<a onclick="setChangePassword(\''+value.email+'\','+value.userId+');"> [Edit]</a>';
					...
					$.ajax({
						...
						success:function(response){
							if(response!=null){
								...
								{
								 var teacherDetail='';
								 teacherDetail+='<thead><tr><th>Crim Id</th>';
								 teacherDetail+='<th>Email</th>';
								 teacherDetail+='<th>Teacher Name</th>';
									 teacherDetail+='<th>School Name</th>';
								 teacherDetail+='<th>Role</th>'+	
								 '<th>Action</th>'+
									'<th style="display:none;"></th><th style="display:none;"></th></tr></thead><tbody>';
									...
									$.each(response.modelMap.migrated, function(index,value) 
										{
											...
											if(userRoleId==2 && status=='Y'){
												if(value.isBlocked == 'N'){
											teacherDetail+='<tr><td>'+value.userId+'</td>';
											teacherDetail+='<td>'+value.email+'</td>';
											teacherDetail+='<td>'+value.teacherName+'</td>';
											teacherDetail+='<td>'+value.schoolName+'</td>';
											
											if(value.roleType==undefined){
												teacherDetail+='<td id=status-'+value.userId+'></td>';
											}
											else
												{
												teacherDetail+='<td id=status-'+value.userId+'>'+value.roleType+'</td>';
												}
											teacherDetail+='<td>'+
											 '<a onclick="setTeacherProgress(\''+value.email+'\','+value.userId+');">[Set Progress] </a>'+
											 '<a onclick="deleteTeacher('+value.userId+');">[Delete] </a>'+
				...
				function changePasswordFrom(){
					...
					var url='<%=request.getContextPath()%>/superadmin/setUserDetails';
					
						$.ajax({
							url:url,
							method:'POST',
							async :false,
							data:{teacherUserId:teacherUserId,newPass:newPass,email:email},
							success:function(response){
...
<script type="text/javascript">
	...
	(function($){
    		$('#prischoolName').autocomplete({
    	        serviceUrl: '<%=request.getContextPath()%>/searchCrimSchool',
    	        paramName: "name",
    	        delimiter: ",",
    	        delay: 3,
    	        minChars: 1,
    		        	...
    		        	// $("#address").val(event.data);
    		        	 ...
    		        	 $.ajax({
    		     			...
    		     			success : function(result) {
    		     				...
    		     				$("#taddress").val(result.modelMap.dtoTeacherSignOn.address);
...
function savePrincipal()
{
	...
	if($('#signupForm').valid()){
	$.ajax({
		url : "<%=request.getContextPath()%>/superadmin/principalSignupCrim",
		method : 'POST',
		headers: { 
			'Content-Type':'application/json'
			},
...
function saveTeacher()
{
	...
	$.ajax({
		url : "<%=request.getContextPath()%>/superadmin/teacherSignupCrim",
		method : 'POST',
		headers: { 
			'Content-Type':'application/json'
			},
		data : JSON.stringify({"email":email,"firstName":firstName ,"address":address,"city":city,"zipCode":zipCode,"password":password,"schoolName":schoolName,"state":state,"inviteDate":inviteDate,"programIds":programIds,"className":className}),
		async:false,
		success : function(result) {
