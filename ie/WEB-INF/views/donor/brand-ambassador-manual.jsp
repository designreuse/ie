			...
			<div class="container">
				...
				<div class="procod">
					...
					<%-- <div class="IE-sub-link">
						<a href="<%=request.getContextPath()%>/donor/learnMore">Learn More</a>
					</div> --%>
				</div>
				<div>
				<div class="wdth50 fl">
                          <span>Search near me(zipcode): </span>
                           <input type="text" name="newZip" id="newZip" value="${zipCode}" onchange="getZipLatLong();" style="width:60px; border: 1px solid #ccc; margin-bottom: 0px;" title="Add zipcode" /> 
                          <img src="<%=request.getContextPath()%>/NewStyles/images/search.png"  alt=""/>
                          </div>
                          										<div class="IE-sub-link wdth50 fl ">
	   ...
	   function(results_array, status) { 
	                        ...
	                        /* if (status === google.maps.GeocoderStatus.OK) { */
	                         ...
	                         var lng = results_array[0].geometry.location.lng();
	                                var url = '<%=request.getContextPath()%>';
	                                $.ajax({
	                                                url : url+'/donor/brand-ambassador-manual-result',
	                                                type: "POST",
	                                                ...
	                                                success : function(result) {
	                                					...
	                                					var fav = '';
	                                					var reachOut ='';
	                                					$.each(result.modelMap.dtoParent, function(index,value) {
	                                						fav = '';
	                                						reachOut ='';
	                                						if(value.isFavorite == 'Y'){
	                                							fav = '<a id="fav-'+value.schoolId+'" onclick="deleteFavorite(\''+value.schoolId+'\',\''+value.isIESchool+'\');"><img src="'+url+'/NewStyles/images/red-hrt.png"></a>';
	                                						}else{
	                                							fav = '<a id="fav-'+value.schoolId+'" onclick="deleteFavorite(\''+value.schoolId+'\',\''+value.isIESchool+'\');"><img src="'+url+'/NewStyles/images/grey-hrt.png"></a>';
	                                						}
	                                						
	                                						if(value.isIESchool == 'N'){
	                                							if(value.isPetition == 'Y'){
	                                								if(value.joinPetition == 'N'){
	                                								reachOut += '<input type="button" value="Reach Out" class="startBtn-Clkur" onclick="window.location.href=\''+url+'/donor/petition?schoolId='+value.schoolId+'\'">';
	                                								}
	                                							}else{
	                                								reachOut += '<input type="button" value="Reach Out" class="startBtn-Clkur" onclick="window.location.href=\''+url+'/donor/petition?schoolId='+value.schoolId+'\'">';
	                                							}
	                                						}else{
	                                							reachOut += '<a class="text-green-link" onclick="sendMailToAll(\''+value.contactEmail+'\');">Send Email</a>';
	                                						}
	                                						parentActiveUsers+='<tr><td>'+value.schoolName+'</td><td>'+value.schoolAddress+'</td><td>'+fav+'</td><td class="txtcenter_align">'+reachOut+'</td></tr>';
	                                					 });
	                                					
	                                					parentActiveUsers+='</tbody>';
	                                			$("#sample_7").html(parentActiveUsers);
	                            	            $('#sample_7').dataTable({
	                            	                "aoColumns": [
	                            	                  { "bSortable": true },
	                            	                  { "bSortable": true },
