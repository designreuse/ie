    ...
    <div class="inner_wrapper"> 
        ...
        <div class="pracProgress_hdr">
            <h2>Manage Campaign</h2>
               <div class="green_line_btm"></div>
                ...
                <!-- <div class="check-select-out">
                 ...
                 <p>For $ 100 you can get Lifetime access to our Programs, before May, 2016</p>
                 </div></div> -->
            
            <%-- <c:if test="${fn:length(campaign) le 0}"> --%>
            <div class="fullrow txtcenter_align mrg_tp20 mrg_btm25">
	            ...
	            <a class="blueBtn dis_inl_bl" href="<%=request.getContextPath()%>/principal/school-campaign">START A NEW CAMPAIGN</a>
            </div>
            <%-- </c:if> --%>
                    ...
                    <div class="content_inner">
                        ...
                        <div class="gps_content">
                        	<h3>${campaign.title}</h3>
                            	<p id="campDesc_${campaign.campaignId}" style="padding:10px;">${campaign.description}</p>
                            	<textarea id="updatedCampText_${campaign.campaignId}" style="display: none;" class="man-cam-txt"></textarea>
                            	<a id="editText_${campaign.campaignId}" onclick="editCampDesc(${campaign.campaignId});" class="edt_txt edt-txt-icon">Edit</a> 
                             <form action="camp">
                            <input type="submit" value="Details" class="submit-btn fl" />
                            <!-- <a href="javascript:;" class="donate_now_btn">Details</a> -->
...
/*                 		 if(Number(totalDonationAmount)>(Number(totalAmount)-Number(schoolConhtribution))){
                			 ...
                			 resultPer3 = (100-(resultPer1+resultPer2)-1);
                		 }
                		 else{
                			 resultPer3 = 100-(resultPer1+resultPer2);
                		 } */
...
<script language="javascript">
				...
				function editCampDesc(campId){
					var editText = $("#editText_"+campId).text();
					
					if(editText == "Edit"){
						$("#editText_"+campId).addClass('sv-txt-icon');
						$("#editText_"+campId).removeClass('edt-txt-icon');
						$("#editText_"+campId).text("Save");
						var campDesc = $("#campDesc_"+campId).text();
						
						$("#updatedCampText_"+campId).val(campDesc);
						$("#updatedCampText_"+campId).show();
						$("#campDesc_"+campId).hide();
						
					}
					else if(editText == "Save"){
						$("#editText_"+campId).addClass('edt-txt-icon');
						$("#editText_"+campId).removeClass('sv-txt-icon');
						$("#editText_"+campId).text("Edit");
						
						var campDesc = $("#updatedCampText_"+campId).val();
						
						$("#campDesc_"+campId).text(campDesc);
						$("#updatedCampText_"+campId).hide();
						$("#campDesc_"+campId).show();
						
						
						
						var url='<%=request.getContextPath()%>/principal/updateCampDesc';
						$.ajax({
							url:url,
							method:'POST',
							async :false,
							data:{campId:campId,campDesc:campDesc},
							cache:false,
