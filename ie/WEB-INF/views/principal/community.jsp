                           ...
                           <!-- <div class="col-md-3 col-xs-3 avatar p0">
                            ...
                            <div class="col-md-9 col-xs-9 p0">
                                <div class="messages msg_sent">
                                    <p class="more">Teacher played Track </p>
   ...
   $(".ulhar")[0].style.display="none"
    ...
    $('#t1').on('click',function(){
 	...
 	function getActivityFeedData(){
 		$("#activityFeedDiv").html('');
 		
		var url='<%=request.getContextPath()%>/principal/getActivityFeedData';
		//alert("getActivityFeedData called");
		$.ajax({
			url:url,
			method:'GET',
			async :false,
			...
			success:function(response){
				...
				var divData = '';
          		$.each( response.modelMap.dtoAudioTrackDetails, function(index,value) {
          			divData+='<div class="col-md-3 col-xs-3 avatar p0">'+
                    	'<img class="media-object-activity" src="http://innerexplorer.org/ie/NewStyles/images/school-profile/he-icon_763.png" alt="profile">'+
                		'</div>'+
                		'<div class="col-md-9 col-xs-9 p0">'+
                    	'<div class="messages msg_sent">'+
                        '<p class="more">'+value.userName+' played '+value.audioTitle+ '. </p>'+
                        '<time datetime="2009-11-13">'+value.createdDate+'</time>'+
                    	'</div>'+
                		'</div>';       
				});
                	
                $("#activityFeedDiv").html(divData);              
				}
		});
 	}
	</script>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
	...
	function getData(forumId){
		...
		var url='<%=request.getContextPath()%>/principal/getForum';
		//alert(forumId);
		$.ajax({
			url:url,
			method:'GET',
          ...
          '</div>'+
          '<div class="outerFourmNew"><div class="clear1"></div>';
          $.each( response.modelMap.dtoForumTopics, function(index,value) {
             divData+='<div class="tab-pane active" id="most-recent">';  
                ...
                '</ul> </div>  </li> </ul> </div>';
                	...
                	divData+='<div class="widget-area no-padding blank"></div>'+
	...
	function SaveTopic(forumId){
		...
		var url='<%=request.getContextPath()%>/principal/saveTopic';
