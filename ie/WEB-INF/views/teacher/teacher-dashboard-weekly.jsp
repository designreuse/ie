...
.screenshot{width: 100%;float: left;text-align: center;}
.screenshot img{max-width:1152px;margin-left: -196px;}
.outer-grn-list .new-center{width: 100% !important;}
.transction-btn{width:100%;float:left;text-align: center !important;}
.transction-button{float: none !important;border:1px solid #02a451 ;
border-radius:20px; background-color: transparent !important;
padding: 7px 18px;color:#fff;font-size: 14px;text-transform: uppercase;
}
</style>
</head>
					...
					<div class="misc_content">
						...
						<div id="tab2">
									...
									<div class="outer-grn-list">
										<div class="right-grn-list">
											...
											<div id="container"
												...
												<%-- <div class="col-md-12 transction-btn">
											<input class="transction-button hdr-btn-clr " value="Bonus Track" style="cursor:pointer" type="button" onclick="location.href = '<%=request.getContextPath()%>/teacher/bonusTracks';">
										    </div> --%>
							...
							<div class="ie_pra-player">
								<div class="container">
										<div class="col-md-12 transction-btn" style="text-align:left !important">
											<input class="transction-button hdr-btn-clr " value="Daily Tracks" style="cursor:pointer;" type="button" onclick="location.href = '<%=request.getContextPath()%>/teacher/mindful-practice-new';">
											<input class="transction-button hdr-btn-clr " value="Transition Tracks" style="cursor:pointer; background-color: white !important; color: #02a451;" type="button" onclick="location.href = '<%=request.getContextPath()%>/teacher/bonusTrack';">
										    </div>
									<div id="circle"></div>
											...
											<div class="${selectItem} position-relative"
												...
												<a href="javascript:;">
													...
													<div class="ie_ply_img">
													    ...
													    <c:if test="${audio.key.programId eq '1'}" > 
														<source src="${bucketPath}bitrate/lowbitrate/Exploring-Me/English/${audio.key.audioName}"
																type="audio/mpeg" data-refid="${audio.key.audioId}"/>
														</c:if>
														<c:if test="${audio.key.programId eq '2'}" > 
														<source src="${bucketPath}bitrate/lowbitrate/Exploring-Originality/English/${audio.key.audioName}"
																type="audio/mpeg" data-refid="${audio.key.audioId}"/>
														</c:if>
														<c:if test="${audio.key.programId eq '3'}" > 
														<source src="${bucketPath}bitrate/lowbitrate/Exploring-Potential/English/${audio.key.audioName}"
																type="audio/mpeg" data-refid="${audio.key.audioId}"/>
														</c:if>
														<c:if test="${audio.key.programId eq '4'}" > 
														<source src="${bucketPath}bitrate/lowbitrate/Exploring-Relevance/English/${audio.key.audioName}"
																type="audio/mpeg" data-refid="${audio.key.audioId}"/>
																</c:if>
													<%-- 	<c:if test="${audio.key.programId eq '4'}" > 
														<source src="${bucketPath}bitrate/lowbitrate/Home-Edition/English/${audio.key.audioName}"
																type="audio/mpeg" data-refid="${audio.key.audioId}"/>
														</c:if> --%>
														</audio>
